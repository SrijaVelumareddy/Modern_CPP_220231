...
//Any access to a shared resource is the critical section
void Square(int number){
    mt.lock(); //ask for the permission to access out
    std::cout << number * number << "\n"; //crirical section
    mt.unlock(); //release the lock when done
}

//Order execution is not guaranteed because we don't know which thread will execute first. If you want your output in ordered way, then don't use threads.
